...
static void Create4Screen(Engine* game, bool OnOneWindow = true)
{
    ...
    else
    {
        ...
        int32_t size = GetSystemMetrics(SM_CYSCREEN) / 2 - space;
        int32_t offset = (GetSystemMetrics(SM_CXSCREEN) - 3 * size) / 2;
        auto cam1 = game->CreateComponent<SpringArmComponent>(game->CreateCamera(ViewType::OrtXOY));
        auto cam2 = game->CreateComponent<SpringArmComponent>(game->CreateCamera(ViewType::OrtYOZ));
        auto cam3 = game->CreateComponent<SpringArmComponent>(game->CreateCamera(ViewType::OrtXOZ));
        //auto cam4 = game->CreateComponent<SpringArmComponent>(game->CreateCamera(ViewType::General));
        cam1->attachRotate = false;
        cam2->attachRotate = false;
        cam3->attachRotate = false;
        //cam4->attachRotate = false;
...
{
    Planet* planet1 = CreatePlanet(Vector3(0.0f, 2.0f, 0.0f), Vector3(0.0f), Vector3(1.0f), false, Vector3(1.0f, 0.8f, 0.2f), 4.0f, true,
        Vector3(0.30f, 45.0f, 0.0f), true, Keys::D1);
    planet1->meshComponent->attachRotate = false;
    planet1->moveComp->speed = 1.f;

    Planet* planet2 = CreatePlanet(Vector3(-7.0f, 0.0f, 0.0f), Vector3(0.0f), Vector3(1.0f), false, Vector3(0.0f, 1.0f, 0.2f), 1.0f, true,
        Vector3(0.0f, -6.0f, 0.0f), false, Keys::D2);
    planet2->moveComp->speed = 1.0f;
    planet1->AddSputnik(planet2);

    Planet* planet3 = CreatePlanet(Vector3(22.0f, 0.0f, 4.0f), Vector3(00.0f), Vector3(1.0f), false, Vector3(1.0f, 0.4f, 0.9f), 1.0f, true,
        Vector3(0.0f, 5.0f, 0.0f), false, Keys::D3);
    planet1->AddSputnik(planet3);
    planet3->moveComp->speed = -10.0f;

    Planet* planet4 = CreatePlanet(Vector3(7.0f, 0.0f, 0.0f), Vector3(0.0f, 0.0f, 0.0f), Vector3(0.6f), false, Vector3(0.2f, 0.4f, 0.9f),
    ...
    Planet* planet5 = CreatePlanet(Vector3(00.0f, .0f, 4.5f), Vector3(0.0f, 0.0f, 00.0f), Vector3(0.2f), true, Vector3(0.2f, 0.8f, 0.3f),
        1.0f, true, Vector3(-0.0f, -30.0f, 00.0f), false, Keys::D5);
    planet5->moveComp->speed = -10.f;
    planet4->AddSputnik(planet5);

    Planet* planet6 = CreatePlanet(Vector3(00.0f, 4.5f, 4.5f), Vector3(45.0f, 0.0f, 0.0f), Vector3(0.2f), true, Vector3(0.2f, 0.8f, 0.3f),
